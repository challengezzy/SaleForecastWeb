...
/**
 * 
 * <p>Title: mtd相关数据操作类</p>
 * <p>Description: </p>
 * <p>Company: dmdd</p> 
 * @author zzy
 * @date Jul 19, 2017 11:23:13 PM
 */
public class InMtdDataDM extends BasicDM {
    ...
    public List<Map<String,String>> importData4UI(List<Map<String,String>> dataList) throws Exception {
    	...
    	sqlSb.append(" SELECT 1 FROM MTD_DATA T WHERE PERIOD=? AND ORGANIZATIONID=? AND PRODUCTID=? ");
    	sqlSb.append(" ) insert into MTD_DATA(version, period, organizationid, productid, quantity, comments) ");
    	sqlSb.append("  values (0, ?, ?, ?, ?, ? )");
    	sqlSb.append(" else ");
    	sqlSb.append(" update IN_STOCK_DATA set version=version+1, quantity=?, comments=? ");
    	sqlSb.append(" WHERE PERIOD=? AND ORGANIZATIONID=? AND PRODUCTID=? ");
    	sqlSb.append("");
    	
//    	String sql = "insert into MTD_DATA(version, period, organizationid, productid, quantity, comments) values "
//            + "(0, ?, ?, ?, ?, ? )";
    	//String rowResult = "OK";
    	for(Map<String,String> data : dataList){
    		try{
				...
				logger.warn("当月收货导入中，单条数据处理异常["+data+"]");
				e.printStackTrace();
			}
    	}
    	
    	logger.info("导入当月收货数据，本次处理["+dataList.size()+"]条，成功["+okNum+"]条，失败["+failNum+"]条");
    ...
    public List<Map<String,Object>> queryByCondition(Map<String,String> cond) throws Exception {
        String sql = "SELECT D.ID,D.PERIOD,D.PRODUCTID,D.ORGANIZATIONID,D.QUANTITY,D.COMMENTS"
        	...
        	+" FROM MTD_DATA D,PRODUCT P,ORGANIZATION O"
