...
public class StockRiskReportService {
	...
	public Map<String,Object> queryStockRiskReportByProduct(Map<String,String> queryCond ) throws Exception
	{
		...
		boolean isIncludePa = new Boolean(queryCond.get("isPA"));
		...
		if(isIncludePo){
			//预计PO收货日期 		
			poData = makePoPrPaRowDate("PO交货期",period,product, periodBegin, periodEnd, OrderConstant.ORDER_RECEIVING_PERIOD,OrderType.PO.getCode());
			rstList.add(poData);
			
			//查询PO单下架期
			poOffData = makePoPrPaRowDate("PO下架期",period,product, periodBegin, periodEnd,OrderConstant.ORDER_OFFSHELF_PERIOD,OrderType.PO.getCode());
		...
		//预计PR--采购申请收货
		if(isIncludePr){
			prData = makePoPrPaRowDate("PR交货期",period,product, periodBegin, periodEnd, OrderConstant.ORDER_RECEIVING_PERIOD,OrderType.PR.getCode());
			rstList.add(prData);
			//查询PR单下架期
			prOffData = makePoPrPaRowDate("PR下架期",period,product, periodBegin, periodEnd,OrderConstant.ORDER_OFFSHELF_PERIOD,OrderType.PR.getCode());
			rstList.add(prOffData);
		}
		
		ABUiRowData paData = null;
		ABUiRowData paOffData = null;
		//PA
		if (isIncludePa) {
			//PA收货日期
			paData = makePoPrPaRowDate("PA交货期",period,product, periodBegin, periodEnd, OrderConstant.ORDER_RECEIVING_PERIOD,OrderType.PA.getCode());
			rstList.add(paData);
			//PA单下架期
			paOffData = makePoPrPaRowDate("PA下架期",period,product, periodBegin, periodEnd,OrderConstant.ORDER_OFFSHELF_PERIOD,OrderType.PA.getCode());
			rstList.add(paOffData);
		}
		...
		//1:累积可提前销售
		//真实累积库存为负值，表示可以有快到期的库存提前在本期卖掉
		double accSellAhead = 0 ;
		...
		for(int i=0;i<periodNum;i++ ){
			...
			//销售,考虑预测达成率
			double sell = getNonNullPeriodValue(calcRowData, i); 
			...
			double pa = 0; 
			if (isIncludePa) {
				paOff = getNonNullPeriodValue(paOffData, i);
				pa = getNonNullPeriodValue(paData, i);
			}
			
			//2:期间过期库存  
			// 当期要下架(PO_off + PR_off+ PA_OFF + INV_off )-当期销售*预测达成率 - 上期缺货. (和上期缺货没有关系？都缺货了，肯定补到上期已经卖掉了)
			double periodOver = (invOff + poOff + prOff + paOff) - sell - preOutOfStock;
			//3:过期库存
			//periodOver + 上一期accSellAhead
			double over = 0;
			...
			//4:期末有效库存
			//[ 上期末 + 本期到货(P0+PR+PA) - sell(销售) - over(过期)],有效库存小时时，才会产生缺货库存
			double ending =  preEndingStock + po + pr +pa- sell - over;
			
			double subtractSS = ending - ss;
			
			//5:缺货库存
			//[sell(销售) - 上期末(preEndingStock) - 本期到货(P0+PR+PA) ]
			double outOfStock = sell - preEndingStock - po - pr - pa;
			if(outOfStock < 0){
				//货足够卖，没有生产缺货
				outOfStock = 0;
			}
	 ...
	 * @return
	 */
	private ABUiRowData makePoPrPaRowDate(String name,int dataPeriod, BProduct product,int periodBegin, int periodEnd,String periodField,String poPrPA) {
		BBizData poBizData = new BBizData();
		poBizData.setName(name);
		ABUiRowData poData = null;
		try {
			poData = riskDao.getPoPrUiRowData(dataPeriod,product, periodBegin, periodEnd, poBizData, periodField,poPrPA);
		} catch (Exception e) {
			logger.info("makePoPrPaRowDate error:"+e.getMessage());
			poData = new ABUiRowData();
		}
		return poData;
	}
...
//			}
	...
	public Map<String,Object> queryInventoryRiskByProductSummary(Map<String,Object> queryCond ) throws Exception{
		...
		//是否包含PR
		boolean isIncludePr = new Boolean(queryCond.get("isPR").toString()); 
		//是否包含PO
		boolean isIncludePo = new Boolean(queryCond.get("isPO").toString());
		//是否包含PA
		boolean isIncludePa = new Boolean(queryCond.get("isPA").toString());
		
		//是否计算金额数据
		boolean isStandardPrice = new Boolean(queryCond.get("isStandardPrice").toString()); 
		String bizDataType = queryCond.get("bizDataType").toString();
		//是否使用实际价格
		boolean isActualPrice = new Boolean(queryCond.get("isActualPrice").toString()); 
		String bizDataCode = queryCond.get("bizDataCode").toString();
		...
		//转换成线程安全的List,可以支持多线程
		List<ABUiRowData> rstList = Collections.synchronizedList(unsafeList); 
		
		int i=0;
		for(String proCode : proCodeList){
			Map<String,ABUiRowData> riskDataMap = queryStockRiskDataByProduct(proCode,period,achieveRate,
					periodNum,isIncludePr,isIncludePo,isIncludePa,isStandardPrice,bizDataType,
					isActualPrice,bizDataCode,fcValidPeriodNum);
	 ...
	 * @throws Exception
	 */
	public Map<String,ABUiRowData> queryStockRiskDataByProduct(String proCode,int period,int achieveRate,
			int periodNum,boolean isIncludePr,boolean isIncludePo,boolean isIncludePa,boolean isStandardPrice,
			String bizDataType,boolean isAcutalPrice,String bizDataCode, int fcValidPeriodNum) throws Exception{
		...
		if(isIncludePo){
			//预计PO收货日期 		
			poData = makePoPrPaRowDate("PO交货期", period, product, periodBegin, queryPeriodEnd,OrderConstant.ORDER_RECEIVING_PERIOD,OrderType.PO.getCode());
			
			//查询PO单下架期
			poOffData = makePoPrPaRowDate("PO下架期", period, product, periodBegin, queryPeriodEnd,OrderConstant.ORDER_OFFSHELF_PERIOD,OrderType.PO.getCode());
		}
		...
		if(isIncludePr){
			//预计PR--采购申请收货
			prData = makePoPrPaRowDate("PR交货期", period, product, periodBegin, queryPeriodEnd,OrderConstant.ORDER_RECEIVING_PERIOD,OrderType.PR.getCode());
			//查询PR单下架期
			prOffData = makePoPrPaRowDate("PR下架期", period, product, periodBegin, queryPeriodEnd,OrderConstant.ORDER_OFFSHELF_PERIOD,OrderType.PR.getCode());
		}
