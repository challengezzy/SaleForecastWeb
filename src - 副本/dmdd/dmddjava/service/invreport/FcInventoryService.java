...
//		int fcPeriodNum = new Integer(queryCond.get("fcPeriodNum").toString());
//		int hisPeriodNum = new Integer(queryCond.get("hisPeriodNum").toString());
		...
		for(String proCode : proCodeList){
			...
			//获取单产品数据
			FcInvRowDataVo rowData = queryFcInventoryReportByProduct(proCode, period, achieveRate,bizDataCode,fcValidPeriodNum,hisDataCode);
	...
	public FcInvRowDataVo queryFcInventoryReportByProduct(String proCode, int period, int achieveRate
		...
		//查询当期PO(当期的所有PO订单汇总)
		Double periodPoTotal = riskDao.getPeriodPoTotalData(product, period, "RECEIVING_PERIOD","PO");
		...
		//计算过期库存
		computeOverStock(product, period, fcPeriodNum, curStockValid, calcRowData, rowData);
	 ...
	 * @param period
	 * @param periodNum
	 * @param curPeriodStockValid
	 * @param adjustBizDataRowData
	 * @throws Exception
	 */
	private void computeOverStock(BProduct product,int period,int periodNum,Double curPeriodStockValid
		...
		ABUiRowData poOffData =  riskDao.getPoPrUiRowData(period,product, periodBegin, periodEnd, poOffBizData,"OFFSHELF_PERIOD","PO");
